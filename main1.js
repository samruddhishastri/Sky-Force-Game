let scene, camera, renderer, starGeo1, starGeo2, starGeo3, stars, stars1, stars2, loader, airplane, mouse, raycaster, bullets = [], enemy, enemy1, enemy2, last_mov_y=1, last_mov_x=1, last_mov1_y=1, last_mov1_x=1, last_mov2_y=1, last_mov2_x=1,enemy_bullets =[], speed_enemy = 0.01, tm=0;
...
function animate(){
    ...
    bullets.forEach(p=>{
        ...
        if(enemy){
            if(Math.abs(p.position.x - enemy.position.x)<= 0.03 && Math.abs(p.position.z - enemy.position.z)<= 0.03 && Math.abs(p.position.z - enemy.position.z)<= 0.03){
                //console.log("Yes")
                scene.remove(p);
                scene.remove(enemy);
            }
        ...
        if(enemy1){
            if(Math.abs(p.position.x - enemy1.position.x)<= 0.03 && Math.abs(p.position.z - enemy1.position.z)<= 0.03 && Math.abs(p.position.z - enemy1.position.z)<= 0.03){
                //console.log("Yes")
                scene.remove(p);
                scene.remove(enemy1);
            }
        ...
        if(enemy2){
            if(Math.abs(p.position.x - enemy2.position.x)<= 0.03 && Math.abs(p.position.z - enemy2.position.z)<= 0.03 && Math.abs(p.position.z - enemy2.position.z)<= 0.03){
                //console.log("Yes")
                scene.remove(p);
                scene.remove(enemy2);
            }
    ...
    if(enemy){
        if(last_mov_x == 1){
            if(enemy.position.x+speed_enemy < 6){
        ...
        if(last_mov_y == 1){
            if(enemy.position.y+speed_enemy < 3.2){
            ...
            if(enemy.position.y-speed_enemy > -2.2){
                enemy.position.y -= speed_enemy
                last_mov_y = 0;
            }
            else{
    ...
    if(enemy1){
        if(last_mov1_x == 1){
            if(enemy1.position.x+speed_enemy < 6){
        ...
        if(last_mov1_y == 1){
            if(enemy1.position.y+speed_enemy < 3.2){
            ...
            if(enemy1.position.y-speed_enemy > -2.2){
                enemy1.position.y -= speed_enemy
                last_mov1_y = 0;
            }
            else{
    ...
    enemy_bullets.forEach(p=>{
        p.position.x += p.velocity_x;
        p.position.y += p.velocity_y;
        p.position.z += p.velocity_z;
    });
...
function enemy_attack(){
    ...
    loader.load( './jet/source/jet.glb', function ( gltf ) {
        ...
        else if(tm%1200==0 && enemy1){
            b.position.set(enemy1.position.x, enemy1.position.y, enemy1.position.z);
        }
        else if(tm%600==0 && enemy2){
            b.position.set(enemy2.position.x, enemy2.position.y, enemy2.position.z);
        }
        b.scale.set(0.05,0.05,-0.05);
        b.velocity_x = -(enemy.position.x - airplane.position.x)/50;
        b.velocity_y = -(enemy.position.y - airplane.position.y)/50;
        b.velocity_z = -(enemy.position.z - airplane.position.z)/300;
